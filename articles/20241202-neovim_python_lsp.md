...
### ディレクトリをさかのぼって project root を見つけるメソッド
...
ファイルがバッファへロードされた時に自動的に実行される処理を定義する際は [`vim.api.nvim_create_autocmd()`](<https://neovim.io/doc/user/api.html#nvim_create_autocmd()>) メソッドを使います。
